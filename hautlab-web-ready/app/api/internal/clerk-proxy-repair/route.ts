...
const targetProxyUrl = "https://www.hautlabmx.com/__clerk";
...
export async function GET() {
  ...
  if (domain.proxy_url === targetProxyUrl) {
    return NextResponse.json({ ok: true, changed: false, proxy: "www", domain: domain.name ?? null });
  }
  ...
  return NextResponse.json({ ok: true, changed: true, proxy: "www", domain: domain.name ?? null });
}
