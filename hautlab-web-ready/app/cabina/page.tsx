...
import { cabinaContent, visibleCabinaServices } from "@/lib/cabina-content";
import { siteConfig } from "@/lib/siteConfig";
import { buildWhatsAppLink } from "@/lib/whatsapp";

const pageUrl = `${siteConfig.url}/cabina`;
const generalMessage = "Hola, me gustaría recibir información y agendar una valoración para la Cabina Dermatocosmética de HAUTLAB.";
...
const gallerySlots = [
  "Retrato profesional de Karen",
  "Preparación de la cabina",
  "Protocolo no invasivo",
  "Detalles del espacio",
  "Productos organizados",
  "Camilla preparada",
  "Fachada o señalización HAUTLAB",
  "Respaldo médico Dr. Salvador Cordero"
];
...
export default function CabinaPage() {
  const localBusinessJsonLd = {
    ...
    parentOrganization: { "@type": "MedicalClinic", name: "HAUTLAB", url: siteConfig.url },
    address: {
      "@type": "PostalAddress",
    ...
    employee: {
      ...
      worksFor: { "@type": "MedicalClinic", name: "HAUTLAB", url: siteConfig.url }
    },
    medicalDirector: {
      "@type": "Physician",
    ...
    <main className="pb-24 lg:pb-0">
        ...
        <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] gap-10 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <div>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Button asChild size="lg">
                <a href={buildWhatsAppLink(generalMessage)} target="_blank" rel="noreferrer" data-event="cabina_reserve_hero">
                  Reservar valoración <ArrowRight className="h-4 w-4" />
                </a>
              </Button>
              <Button asChild variant="outline" size="lg"><a href="#servicios">Conocer los tratamientos</a></Button>
            </div>
          </div>
          ...
          <div className="relative min-h-[520px] overflow-hidden rounded-[2rem] border border-line bg-[#11100e] shadow-calm">
            <div className="absolute inset-0 grid place-items-center p-8 text-center">
              <div>
                <div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-line bg-white/[0.04] text-champagne"><ImageIcon className="h-6 w-6" /></div>
                <p className="mt-6 text-xs uppercase tracking-[0.22em] text-champagne">Activo fotográfico pendiente</p>
                <p className="mx-auto mt-4 max-w-sm text-sm leading-7 text-muted">Espacio reservado para una fotografía real de Karen dentro de HAUTLAB, con uniforme clínico y encuadre editorial.</p>
              </div>
            </div>
            <div className="absolute inset-x-5 bottom-5 rounded-[1.5rem] border border-line bg-background/88 p-5 backdrop-blur-xl">
              ...
              <p className="mt-2 text-2xl font-medium text-bone">Karen Cruz</p>
      ...
      <section className="border-b border-line bg-background py-20 lg:py-28">
        <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] gap-10 lg:grid-cols-[.75fr_1.25fr] lg:items-start">
          ...
          <div className="space-y-5 text-base leading-8 text-muted">
            ...
            <a href={buildWhatsAppLink("Hola, me gustaría recibir información sobre la Cabina Dermatocosmética de HAUTLAB.")} target="_blank" rel="noreferrer" data-event="cabina_whatsapp_positioning" className="inline-flex items-center gap-2 pt-3 text-sm text-bone transition hover:text-champagne">Solicitar orientación <ArrowRight className="h-4 w-4" /></a>
          </div>
        </div>
      </section>
      ...
      <section id="servicios" className="scroll-mt-28 border-b border-line bg-soft/25 py-20 lg:py-28">
        <div className="mx-auto w-[min(1180px,calc(100%-32px))]">
          ...
          <div className="mt-9 text-center"><Button asChild size="lg"><a href={buildWhatsAppLink(generalMessage)} target="_blank" rel="noreferrer" data-event="cabina_reserve_after_services">Consultar disponibilidad <CalendarDays className="h-4 w-4" /></a></Button></div>
        </div>
      </section>
      ...
      <section className="border-b border-line bg-background py-20 lg:py-28">
        <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          <div className="relative min-h-[460px] overflow-hidden rounded-[2rem] border border-line bg-white/[0.025]">
            <div className="absolute inset-0 grid place-items-center p-8 text-center"><div><UserRound className="mx-auto h-12 w-12 text-champagne" /><p className="mt-5 text-xs uppercase tracking-[0.2em] text-champagne">Retrato pendiente</p><p className="mt-4 max-w-sm text-sm leading-7 text-muted">Este espacio utilizará exclusivamente una fotografía profesional real de Karen dentro de HAUTLAB.</p></div></div>
          </div>
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Button asChild><a href={buildWhatsAppLink("Hola, me gustaría agendar con la Cabina Dermatocosmética de HAUTLAB coordinada por Karen Cruz.")} target="_blank" rel="noreferrer" data-event="cabina_reserve_after_karen">Reservar <ArrowRight className="h-4 w-4" /></a></Button>
      ...
      <section className="border-b border-line bg-background py-20 lg:py-28">
        <div className="mx-auto w-[min(1180px,calc(100%-32px))]">
          <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <Card className="p-7"><p className="text-base leading-8 text-muted">{cabinaContent.reviewsPlaceholder}</p><p className="mt-5 text-xs leading-5 text-quiet">La sección queda preparada para integrar opiniones verificadas sin atribuir a la cabina reseñas generales de HAUTLAB.</p></Card>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-line bg-soft/25 py-20 lg:py-28">
        <div className="mx-auto w-[min(1180px,calc(100%-32px))]">
          <p className="text-xs uppercase tracking-[0.2em] text-champagne">Archivo visual</p><h2 className="mt-4 max-w-4xl font-serif text-[clamp(2.7rem,5vw,4.8rem)] leading-[.94] tracking-[-.055em] text-bone">Espacios preparados para fotografía real, sin recurrir a clichés de spa.</h2>
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{gallerySlots.map((slot) => <div key={slot} className="grid aspect-[4/3] place-items-center rounded-[1.5rem] border border-dashed border-line bg-white/[0.02] p-5 text-center"><div><ImageIcon className="mx-auto h-5 w-5 text-champagne" /><p className="mt-3 text-xs leading-5 text-muted">{slot}</p></div></div>)}</div>
        </div>
      </section>
      ...
      <section className="border-b border-line bg-background py-20 lg:py-28">
        ...
        <div className="mx-auto mt-10 w-[min(1180px,calc(100%-32px))] text-center"><Button asChild size="lg"><a href={buildWhatsAppLink("Hola, me gustaría recibir información sobre la Cabina Dermatocosmética de HAUTLAB.")} target="_blank" rel="noreferrer" data-event="cabina_reserve_after_faq">Reservar por WhatsApp <ArrowRight className="h-4 w-4" /></a></Button></div>
      </section>
      ...
      <section className="bg-bone py-16 text-background">
        <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div><p className="text-xs uppercase tracking-[0.2em] text-taupe">HAUTLAB · San Ramón Norte</p><h2 className="mt-4 font-serif text-[clamp(2.8rem,5vw,4.8rem)] leading-[.94] tracking-[-.055em]">Cabina dermatocosmética dentro de HAUTLAB.</h2><p className="mt-5 max-w-2xl text-sm leading-7 text-[#4b4036]">Calle 43 #299A x 32A, San Ramón Norte, C.P. 97117, Mérida, Yucatán. Atención únicamente con cita previa.</p></div>
          <div className="grid gap-3 sm:grid-cols-2"><a href={buildWhatsAppLink(generalMessage)} target="_blank" rel="noreferrer" data-event="cabina_contact_whatsapp" className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-background px-6 text-sm font-medium text-bone">WhatsApp <ArrowRight className="h-4 w-4" /></a><a href="https://maps.app.goo.gl/8CoPkGjpuYDi8QqE6?g_st=ic" target="_blank" rel="noreferrer" data-event="cabina_contact_maps" className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full border border-background/25 px-6 text-sm font-medium">Ubicación <MapPin className="h-4 w-4" /></a></div>
        </div>
      </section>

      <div className="fixed bottom-3 left-3 right-20 z-40 lg:hidden"><a href={buildWhatsAppLink(generalMessage)} target="_blank" rel="noreferrer" data-event="cabina_mobile_conversion" className="flex min-h-14 items-center justify-between rounded-full border border-champagne/30 bg-champagne px-5 text-sm font-medium text-background shadow-calm"><span>Reservar cabina</span><ArrowRight className="h-4 w-4" /></a></div>
